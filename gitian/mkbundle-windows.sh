...
then
  ...
  ./bin/gbuild -j $NUM_PROCS --commit pyptlib=$PYPTLIB_TAG,obfsproxy=$OBFSPROXY_TAG,flashproxy=$FLASHPROXY_TAG,goptlib=$GOPTLIB_TAG $DESCRIPTOR_DIR/windows/gitian-pluggable-transports.yml
  if [ $? -ne 0 ];
  then
